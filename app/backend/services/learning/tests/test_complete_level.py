...
class TestCompleteLevel(BaseTestSetup):
    ...
    def test_complete_level_with_active_battlepass(self):
        ...
        self.assertTrue(
            any(entry.get('season') == active_season for entry in updated_battlepass),
            f"Battlepass should have an entry for active season {active_season}"
        )
        ...
        active_season_entry = next((entry for entry in updated_battlepass if entry.get('season') == active_season),
                                   None)
        self.assertIsNotNone(active_season_entry, f"Battlepass should have an entry for active season {active_season}")
        actual_bp_xp = Decimal(str(active_season_entry.get('xp', 0)))
        ...
        active_season_entry = next((entry for entry in updated_battlepass if entry.get('season') == active_season),
                                   None)
        self.assertIsNotNone(active_season_entry, f"Battlepass should have an entry for active season {active_season}")
        self.assertIn('claimed_levels', active_season_entry,
                      "Battlepass entry should have a claimed_levels array")
        # print(f"Claimed levels array: {active_season_entry.get('claimed_levels', [])}")
    ...
    def test_with_new_achievement(self):
        ...
        self.assertEqual(response['statusCode'], 200)
        self.assertEqual(body['message'], "Level completed successfully")
        achievement_ids = [achievement['id'] for achievement in body['new_achievements']]
        self.assertIn("new_achievement", achievement_ids, "User should have received the new achievement")
        updated_user = self.users_table.get_item(Key={'email': email})['Item']
    ...
    def test_with_achievements(self):
        ...
        self.sample_achievements = [
            ...
            {
                "id": "words_learned_achievement_1",
                "type": "words",
                "requires": 1
            },
            {
                "id": "words_learned_achievement_2",
                "type": "words",
                "requires": 2
            },
            {
                "id": "words_learned_achievement_3",
                "type": "words",
                "requires": 3
            },
            {
                "id": "words_learned_achievement_123",
                "type": "words",
                "requires": 123
            },
            {
        ...
        achievement_ids = [achievement['id'] for achievement in body['new_achievements']]
        self.assertIn("words_learned_achievement_1", achievement_ids,
                      "User should have received the words_learned_achievement_1")
        self.assertIn("words_learned_achievement_2", achievement_ids,
                      "User should have received the words_learned_achievement_2")
        self.assertIn("words_learned_achievement_3", achievement_ids,
                      "User should have received the words_learned_achievement_3")
        self.assertIn("time_played_achievement", achievement_ids,
                      "User should have received the time_played_achievement")
        self.assertIn("time_played_achievement_2", achievement_ids,
                      "User should have received the time_played_achievement_2")
