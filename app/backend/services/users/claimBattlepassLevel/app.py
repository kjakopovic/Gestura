...
def claim_battlepass_level(user_dynamodb, battlepass_dynamodb, email, claim_level):
    ...
    # Check if the level is unlocked and available for claiming
    ...
    # Recalculate user's current level, unlocked levels, and locked levels
    current_level = 0
    unlocked_levels = []
    locked_levels = []
    current_xp = 0
    ...
    # Update user's battlepass data for the current season
...
def get_active_battlepass_seasons(dynamodb):
    ...
    # If multiple active battlepasses are found, return the first one
